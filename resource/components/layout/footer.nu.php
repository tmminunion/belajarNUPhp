</div>
<div class="container-fluid bg-dark">
    <footer class="d-flex flex-wrap justify-content-between align-items-center py-3">
        <p class="col-md-4 mb-0 text-muted"></p>



        <a href="<?= getBaseUrl(); ?>" class="col-md-4 d-flex mr--5align-items-center justify-content-center mb-1 mb-md-0 me-md-auto link-dark text-decoration-none">
            <img src="<?= getBaseUrl(); ?>assets/img/brand/white.png" class="navbar-brand-img" height="45">
        </a>

        <a href="https://nufat.id" class="nav-link px-2 text-muted">&copy; 2024 nufat.id</a>


    </footer>
</div>


<!--   Core   -->
<script src="<?= getBaseUrl(); ?>assets/js/plugins/jquery/dist/jquery.min.js"></script>
<script src="<?= getBaseUrl(); ?>assets/js/plugins/bootstrap/dist/js/bootstrap.bundle.min.js"></script>

<?= $this['scriptsfooter'] ?>
</body>

</html>